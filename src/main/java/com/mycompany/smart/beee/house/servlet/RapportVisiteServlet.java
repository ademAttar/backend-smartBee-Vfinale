...
import com.mycompany.smart.beee.house.dao.RapportVisiteDAO;
...
@WebServlet("/rapport/*")
public class RapportVisiteServlet extends HttpServlet {
    ...
    public void init() throws ServletException {
        try {
            ...
            rapportDAO = new RapportVisiteDAO(connection, planningDAO, userDAO, rucheDAO);
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) 
        ...
        try {
            ...
            List<RapportVisite> rapports = rapportDAO.findByRuche(rucheId);
            response.getWriter().write(gson.toJson(rapports));
        } catch (NumberFormatException e) {
            response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
            response.getWriter().write("{\"error\": \"ID de ruche invalide\"}");
        } catch (SQLException e) {
            response.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
            response.getWriter().write("{\"error\": \"Erreur lors de la récupération des rapports\"}");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) 
        ...
        try {
            RapportVisite rapport = gson.fromJson(request.getReader(), RapportVisite.class);
            rapportDAO.create(rapport);
            response.setStatus(HttpServletResponse.SC_CREATED);
            response.getWriter().write(gson.toJson(rapport));
        } catch (SQLException e) {
            response.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
            response.getWriter().write("{\"error\": \"Erreur lors de la création du rapport\"}");
        }
    }
}
