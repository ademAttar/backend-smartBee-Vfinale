...
public class RucheServlet extends HttpServlet {
    ...
    public void init() throws ServletException {
        try {
            ...
            rucheDAO = new RucheDAO(connection, userDAO, siteDAO);
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) 
        ...
        if (pathInfo == null || pathInfo.equals("/")) {
            ...
            response.getWriter().write("{\"error\": \"ID de site requis\"}");
            return;
        }

        try {
            Long siteId = Long.parseLong(pathInfo.substring(1));
            List<Ruche> ruches = rucheDAO.findBySite(siteId);
            response.getWriter().write(gson.toJson(ruches));
        } catch (NumberFormatException e) {
            response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
            response.getWriter().write("{\"error\": \"ID de site invalide\"}");
        } catch (SQLException e) {
            response.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
            response.getWriter().write("{\"error\": \"Erreur lors de la récupération des ruches\"}");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) 
        ...
        try {
            Ruche ruche = gson.fromJson(request.getReader(), Ruche.class);
            rucheDAO.create(ruche);
            response.setStatus(HttpServletResponse.SC_CREATED);
            response.getWriter().write(gson.toJson(ruche));
        } catch (SQLException e) {
            response.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
            response.getWriter().write("{\"error\": \"Erreur lors de la création de la ruche\"}");
        }
    }
}
